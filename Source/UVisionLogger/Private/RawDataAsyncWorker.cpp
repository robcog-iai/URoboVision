...
RawDataAsyncWorker::RawDataAsyncWorker(TArray<FColor>& Image_init, TSharedPtr<IImageWrapper>& ImageWrapperRef, FDateTime Stamp, FString Folder, FString Name, int Width_init, int Height_init, FString ImageEpisode, FString CameraId)
{
	Width = Width_init;
	Height = Height_init;
	TimeStamp = Stamp;
...
void RawDataAsyncWorker::SaveImage(TArray<FColor>& image, TSharedPtr<IImageWrapper>& ImageWrapper, FDateTime Stamp, FString Folder, FString ImageName, int Width, int Height,FString Episode, FString CameraId)
{
	// get the time stamp
	...
	FString FileDir = FPaths::ProjectDir() + TEXT("/Vision Logger/")+ Episode + "/" + CameraId + "/"+ TEXT("viewport/") + Folder +"/";
